...
const CardUI = ({
  ...
  if (hidden) {
    return (
      ...
      >
        ...
        <div className="text-white/30 text-3xl md:text-5xl font-serif italic z-10 font-black tracking-tighter drop-shadow-lg">
          BJ
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="font-bold text-sm sm:text-lg md:text-xl leading-none z-10 drop-shadow-sm">
        {card.rank}
      </div>
      <div className="self-center text-3xl sm:text-5xl z-10 drop-shadow-md">
        {getSuitSymbol(card.suit)}
      </div>
      <div className="font-bold text-sm sm:text-lg md:text-xl self-end rotate-180 leading-none z-10 drop-shadow-sm">
        {card.rank}
      </div>
    </div>
  );
      ...
      <div className="flex justify-between items-center p-4 sm:p-6 pb-4 border-b border-white/5 shrink-0">
        <div className="flex flex-col">
          <h2 className="text-xl sm:text-2xl font-black italic tracking-tighter flex items-center gap-2 drop-shadow-2xl">
            ...
            <span className="bg-gradient-to-b from-white to-emerald-200 bg-clip-text text-transparent">
              THE TABLE
            </span>
          </h2>
        </div>
        ...
        <div className="flex flex-col items-end gap-1">
          <div className="bg-black/40 pl-4 pr-1 py-1 rounded-full flex items-center gap-3 border border-white/5 shadow-inner backdrop-blur-md">
            <span className="text-emerald-400 font-bold uppercase text-[9px] tracking-[0.2em]">
              Bankroll
            </span>
            <div className="bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">
              <span className="text-lg sm:text-xl font-mono font-bold text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]">
                ${bankroll}
              </span>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col justify-center sm:justify-evenly gap-2 sm:gap-4 p-4 sm:p-6 relative min-h-0 overflow-hidden">
        ...
        <div className="flex flex-col items-center gap-2 z-10">
          <div className="flex items-center gap-2 text-emerald-300/60 italic">
            ...
            <span className="uppercase text-[9px] sm:text-xs font-black tracking-[0.2em] drop-shadow-sm">
              The Dealer
            </span>
            {gameState !== "betting" && (
              <span className="bg-white/5 px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-mono border border-white/10 backdrop-blur-md">
                {gameState === "player_turn" && dealerHand.length > 0
                  ? "??"
          ...
          <div className="flex justify-center h-24 sm:h-32 md:h-36">
            {dealerHand.map((card, i) => (
              <div
                key={i}
                className={`animate-in fade-in slide-in-from-top-8 duration-500 relative ${i > 0 ? "-ml-4 sm:-ml-6 md:-ml-8" : ""}`}
              >
        ...
        <div className="text-center py-1 z-20 h-10 flex items-center justify-center">
          <div
            className={`inline-block bg-white/5 backdrop-blur-2xl px-6 py-1.5 sm:px-10 sm:py-2 rounded-full text-base sm:text-lg font-black border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.5)] text-white tracking-widest transition-all duration-300 ${gameState === "result" ? "scale-110 bg-emerald-500/10 border-emerald-500/30 font-black" : "animate-pulse"}`}
          >
            {message}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-2 z-10">
          <div className="flex justify-center h-24 sm:h-32 md:h-36">
            {playerHand.map((card, i) => (
              <div
                key={i}
                className={`animate-in fade-in slide-in-from-bottom-8 duration-500 relative ${i > 0 ? "-ml-4 sm:-ml-6 md:-ml-8" : ""}`}
              >
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-between w-full max-w-3xl mx-auto">
            {bankroll < 10 && currentBet > bankroll ? (
              <div className="flex flex-col items-center gap-4 w-full">
                <p className="text-rose-400 font-black text-base italic tracking-tight drop-shadow-2xl">
                  OUT OF CHIPS
                </p>
                <button
                  onClick={() => {
                  ...
                  {[10, 50, 100, 500].map((amt) => {
                    const getChipColor = (value: number) => {
                      switch (value) {
                        ...
                        case 500:
                          return "bg-yellow-500 border-yellow-300 text-yellow-950";
                        default:
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`absolute inset-0.5 border border-dashed rounded-full opacity-30 ${amt === 500 ? "border-yellow-900" : "border-white"}`}
                        ></div>
